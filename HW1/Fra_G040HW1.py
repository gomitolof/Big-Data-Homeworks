...
def online_retailer_1(docs, S):
    productCustomer = (docs.flatMap(lambda x: pid_cid_per_tansaction(x, S)) # <-- MAP PHASE (R1)
        .reduceByKey(lambda x, y: x)            # <-- REDUCE PHASE (R1)
...
# CHECKING NUMBER OF CMD LINE PARAMTERS
    ...
    # printList = productPopularity1.collect()
    # for i in range(len(printList)):
    #     print("Product: %s Popularity: %s" %(printList[i][0], printList[i][1]), end ="; ")
    # print()
    ...
    # printList = productPopularity2.collect()
    # for i in range(len(printList)):
    #     print("Product: %s Popularity: %s" %(printList[i][0], printList[i][1]), end ="; ")
    # print()
